...
function calculaDist(vetor){
    ...
    vetor.foreach(function(element, index){
        if(index+1<vetor.length){
            novoVetor.push({o: vetor[index],d: vetor[index+1],dist: parseFloat((google.maps.geometry.spherical.computeDistanceBetween(new google.maps.LatLng(vetor[index]),new google.maps.LatLng(vetor[index+1]))))});
        }
    })
    return novoVetor;   
}
//Funçao que calcula e armazena distancia entre pessoa e a posiçao do ponto
function posPessoaPonto(posPessoa, posPonto, rota){
    let indicePonto = 0;
    let indicePessoa = 0;
    let menorDistPonto = google.maps.geometry.spherical.computeDistanceBetween(new google.maps.LatLng(posPonto),new google.maps.LatLng(rota[0].o));
    let menorDistPessoa = google.maps.geometry.spherical.computeDistanceBetween(new google.maps.LatLng(posPessoa),new google.maps.LatLng(rota[0].d));
    let somaPP = 0;
    let distPonto = 0;
    let distPessoa = 0;

    rota.foreach(function(element, index){
        distPonto = google.maps.geometry.spherical.computeDistanceBetween(new google.maps.LatLng(posPessoa),new google.maps.LatLng(rota[index].o));

        distOnibus = google.maps.geometry.spherical.computeDistanceBetween(new google.maps.LatLng(posOnibus),new google.maps.LatLng(rota[index].d));
        if(distPonto < menorDistPonto){
            indicePonto = index;
            menorDistPonto = distPonto;
        }else if(distPessoa < menorDistPessoa){
            indicePessoa = index;
            menorDistPessoa = distPonto;
        }
    })
    //for que percorre do indice da pessoa até o indice do ponto somando as distâncias
    for(let i = indicePessoa; i<=indicePonto; i++){
        somaPP += vetor[i].dist;
    }   
}
//Funçao que calcula a distancia entre a posição do ponto e a posição do onibus
function posPontoOnibus(menorDistPonto,posOnibus,rota){
    let indiceOnibus = 0;
    let distOnibus = 0;
    ...
    rota.foreach(function(element, index){
        if(distOnibus<menorDistOnibus){
            indiceOnibus = index;
            menorDistOnibus = distOnibus;
            
        }
    })    
    //for que percorre do indice do ponto até o indice do onibus somando as distâncias
    for(let i = indicePonto; i<=indiceOnibus; i++){
        somaPO += vetor[i].dist;
    }
    
    //Divide a distância pela velocidade média para encontrar o tempo
    tempo = (somaPP + somaP0/11.11)/60;
    tempo = tempo.toFixed(1);
    return tempo;
}

//posicoes aleatórias para pessoa, onibus e ponto
let posPessoa = {lat:-21.755124,lng:-41.323433};
let posOnibus = {lat:-21.724017,lng:-41.331606};
let posPonto = {lng:-41.3309436,lat:-21.7253521};
